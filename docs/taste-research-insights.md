...
6. **Privacy story is half-told.** Local `taste.db` is good, but inference still goes to Claude/OpenAI. Forums care about this nuance. The `redactSecrets + cap evidence + hash provenance + no remote learning` setup is correct — surface it in `why`/`export` so users *see* nothing raw is stored.
