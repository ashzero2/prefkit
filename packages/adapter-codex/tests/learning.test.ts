...
const hookPath = join(process.cwd(), "packages/adapter-codex/scripts/prefkit-learn.mjs");

describe("Codex learning hook", () => {
  ...
  it("builds a bounded learner event from Codex metadata", () => {
  ...
  it("queues a strong prompt and starts the worker without producing hook output", async () => {
    const root = mkdtempSync(join(tmpdir(), "prefkit-codex-learning-"));
