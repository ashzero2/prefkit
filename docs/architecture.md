...
Retrieval is deterministic. The store uses SQLite FTS5 for the first candidate set, applies status/confidence/scope filters, ranks candidates in core code, and renders a context block that fits the configured injection budget. Retrieval does not call a local or remote model. The renderer estimates tokens as `ceil(characters / 4)`; this heuristic can under- or over-count for code and non-English text, so the configured token budget is a soft bound rather than an exact one. An optional reminder header (`injection.includeHeader`, or `--with-header`) replaces the default header line, and an optional usage boost (`injection.usageHalfLifeDays`) nudges recently injected rules up the ranking; it decays with the half-life and never demotes or deletes a rule. Counting a rule as reused happens in `recordContext`, so it requires metrics to be enabled and never turns a read into a write.
