...
export function buildLearnerEvent(input, env = process.env) {
  ...
  return {
    ...
    userPrompt: prompt,
    assistantSummary: "Codex captured this user prompt before model dispatch.",
    repoContext: {},
    metadata: {
      source: "codex-user-prompt-submit-hook",
...
async function main() {
  ...
  if (input === null || !shouldQueuePrompt(input.prompt) || process.env.PREFKIT_CODEX_QUEUE_EVENTS === "false") {
    return;
  }
  ...
  try {
    ...
    if (result.stdout.includes("queued=true") && env.PREFKIT_CODEX_AUTO_START_WORKER !== "false") {
...
function timeoutMs(env) {
  const configured = Number(env.PREFKIT_CODEX_LEARN_TIMEOUT_MS);
