...
export async function runDoctor(loadResult: ConfigLoadResult): Promise<DoctorReport> {
  ...
  checks.push({
    ...
    ok: existsSync(dirname(config.store.path)),
    message: existsSync(dirname(config.store.path))
      ? "Store directory exists: " + dirname(config.store.path)
      : "Store directory does not exist yet: " + dirname(config.store.path) + ". Run prefkit init in Phase 1.",
  });
...
async function localModelCheck(config: ConfigLoadResult["config"]["localModel"]): Promise<DoctorCheck> {
  ...
  return {
    name: "local-model",
    ok: health.ok,
    message: health.provider + ":" + health.model + " - " + health.message + (health.latencyMs === undefined ? "" : " (" + health.latencyMs + " ms)"),
  };
}
