...
const hookPath = resolve(process.cwd(), "packages/adapter-codex/scripts/prefkit-context.mjs");

describe("Codex context hook", () => {
  it("returns bounded context and passes Codex metadata to prefkit", async () => {
    const root = mkdtempSync(join(tmpdir(), "prefkit-codex-hook-"));
  ...
  it("can emit an optional user-visible system message", async () => {
    ...
    const result = await runHook(
      ...
      { PREFKIT_COMMAND: command, PREFKIT_CODEX_NOTIFY: "true" },
    );
