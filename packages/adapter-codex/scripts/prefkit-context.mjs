...
/**
 * Parse and validate the subset of Codex's UserPromptSubmit payload that the
...
export function buildContextArgs(input, env = process.env) {
  const prompt = truncate(input.prompt, positiveInteger(env.PREFKIT_CODEX_MAX_PROMPT_CHARS, DEFAULT_MAX_PROMPT_CHARS));
  const args = [
    ...prefkitBaseArgs(env),
    "context",
    "--prompt",
    ...
    input.cwd,
    "--limit",
    String(positiveInteger(env.PREFKIT_CODEX_CONTEXT_LIMIT, DEFAULT_LIMIT)),
    "--min-confidence",
    String(numberInRange(env.PREFKIT_CODEX_MIN_CONFIDENCE, DEFAULT_MIN_CONFIDENCE, 0, 1)),
  ];
...
export function buildHookOutput(context, env = process.env) {
  const boundedContext = truncate(
    context.trim(),
    positiveInteger(env.PREFKIT_CODEX_MAX_CONTEXT_CHARS, DEFAULT_MAX_CONTEXT_CHARS),
  );
  ...
  if (env.PREFKIT_CODEX_NOTIFY === "true") {
...
function timeoutMs(env) {
  const configured = Number(env.PREFKIT_CODEX_CONTEXT_TIMEOUT_MS);
...
function debug(message) {
  if (process.env.PREFKIT_CODEX_DEBUG === "true") {
